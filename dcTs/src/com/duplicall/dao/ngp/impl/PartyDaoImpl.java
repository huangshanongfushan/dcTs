...
public class PartyDaoImpl implements IPartyDao {
    ...
    {
        String sql =
            "UPDATE party_identity d SET d.ip_address = '" + party.getIpAddress() + "', d.generic_digits = '" + party.getGenericDigits()
                + "' WHERE d.id IN ( SELECT e.id FROM ( SELECT c.id FROM laes_messages a, answer_messages b, party_identity c WHERE a.laes_call_id = " + callId
        ...
        throws Exception
    {
        String sql =
            "update party_identity d set d.ip_address='" + party.getIpAddress() + "',d.generic_digits='" + party.getGenericDigits()
                + "' where d.id in (select e.id from ( SELECT c.id FROM laes_messages a, origination_messages b, party_identity c WHERE a.laes_call_id = " + callId
                + " AND a.message_type = 'ORIGINATION' AND b.id = a.id AND b.calling_network_party_id = c.id) e)";
        ...
        throws Exception
    {
        String sql =
            "update party_identity d set d.ip_address='" + party.getIpAddress() + "',d.generic_digits='" + party.getGenericDigits()
                + "' where d.id in (select e.id from ( SELECT c.id FROM laes_messages a, origination_messages b, party_identity c WHERE a.laes_call_id = " + callId
                + " AND a.message_type = 'ORIGINATION' AND b.id = a.id AND b.called_network_party_id = c.id) e)";
