...
(function () {
	...
	y = l.jQuery,
	p = l.jQuery,
	o = l.jQuery = l.jQuery = function (E, F) {
		...
		curCSS : function (I, F, G) {
			...
			if (!G && E && E[F]) {
				...
				if (q.getComputedStyle) {
					...
					F = F.replace(/([A-Z])/g, "-jQuery1").toLowerCase();
					...
					if (I.currentStyle) {
						...
						if (!/^\d+(px)?jQuery/i.test(L) && /^\d/.test(L)) {
							var H = E.left,
			...
			o.each(F, function (P, S) {
				...
				if (typeof S === "string") {
					S = S.replace(/(<(\w+)[^>]*?)\/>/g, function (U, V, T) {
							return T.match(/^(abbr|br|col|img|input|link|meta|param|hr|area|embed)jQuery/i) ? U : V + "></" + T + ">"
						});
			...
			if (J.tagName) {
				...
				if (G in J && H && !F) {
					...
					if (G == "tabIndex") {
						...
						return I && I.specified ? I.value : J.nodeName.match(/(button|input|object|select|textarea)/i) ? 0 : J.nodeName.match(/^(a|area)jQuery/i) && J.href ? 0 : g
					}
					return J[G]
				}
		...
		var I = F.selectors = {
			...
			filter : {
				...
				ATTR : function (Y, W) {
					...
					X = W[2],
					U = W[4];
					return T == null ? X === "!=" : X === "=" ? Z === U : X === "*=" ? Z.indexOf(U) >= 0 : X === "~=" ? (" " + Z + " ").indexOf(U) >= 0 : !U ? Z && T !== false : X === "!=" ? Z != U : X === "^=" ? Z.indexOf(U) === 0 : X === "jQuery=" ? Z.substr(Z.length - U.length) === U : X === "|=" ? Z === U || Z.substr(0, U.length + 1) === U + "-" : false
				},
	...
	o.event = {
			...
			if (G) {
				if (H === g || (typeof H === "string" && H.charAt(0) == ".")) {
					...
					o.each(H.split(/\s+/), function (M, O) {
						var Q = O.split(".");
						O = Q.shift();
						var N = RegExp("(^|\\.)" + Q.slice().sort().join(".*\\.") + "(\\.|jQuery)");
						if (G[O]) {
							if (J) {
								delete G[O][J.guid]
							} else {
		...
		handle : function (K) {
			...
			var I = RegExp("(^|\\.)" + L.slice().sort().join(".*\\.") + "(\\.|jQuery)");
	...
	o.extend({
		...
		ajax : function (M) {
			...
			if (M.dataType == "json" && (M.data && M.data.match(F) || M.url.match(F))) {
				W = "jsonp" + r++;
				if (M.data) {
					M.data = (M.data + "").replace(F, "=" + W + "jQuery1")
				}
				M.url = M.url.replace(F, "=" + W + "jQuery1");
				M.dataType = "script";
				l[W] = function (X) {
					V = X;
					I();
			...
			if (M.cache === false && G == "GET") {
				var E = e();
				var U = M.url.replace(/(\?|&)_=.*?(&|jQuery)/, "jQuery1_=" + E + "jQuery2");
			...
			return this[E.queue === false ? "each" : "queue"](function () {
				...
				o.each(I, function (O, S) {
					...
					if (/toggle|show|hide/.test(S)) {
						...
						var Q = S.toString().match(/^([+-]=)?([\d+-.]+)(.*)jQuery/),
						T = R.cur(true) || 0;
						if (Q) {
		...
		o.fn.offset = function () {
			...
			while ((J = J.parentNode) && J !== K && J !== H) {
				...
				if (J === G) {
					N += J.offsetTop,
					I += J.offsetLeft;
					if (o.offset.doesNotAddBorder && !(o.offset.doesAddBorderForTableAndCells && /^t(able|d|h)jQuery/i.test(J.tagName))) {
	...
	o.fn.extend({
		position : function () {
			...
			if (this[0]) {
				...
				J = this.offset(),
				E = /^body|htmljQuery/i.test(G[0].tagName) ? {
					top : 0,
					left : 0
				}
				 : G.offset();
		...
		offsetParent : function () {
			...
			while (E && (!/^body|htmljQuery/i.test(E.tagName) && o.css(E, "position") == "static")) {
				E = E.offsetParent
			}
			return o(E)
		}
