(function(a) {
	...
	var c = {
				...
				if (/(\.jpg|\.png|\.gif|\.bmp|\.jpeg)$/i.test(d)) {
					if (c.cacheData[d]) {
						...
						f.onload = function() {
							...
							var n = '<img class="float_ajax_image" src="' + d
									+ '" width="' + m + '" height = "' + q
									+ '" />';
							c.cacheData[d] = n;
							c.target = a(n);
					...
					if (d) {
						if (c.cacheData[d]) {
							c.target = a('<div class="float_ajax_data">'
									+ c.cacheData[d] + "</div>");
							c.targetShow()
						} else {
							c.loading();
			...
			if (e !== "common") {
				...
				if (d === "plugin") {
					if (!a("#floatBox_" + e).size()) {
						a(
								'<div id="floatBox_' + e + '" class="float_'
										+ e + '_box"></div>').appendTo(
								a("body")).hide()
					}
					d = a("#floatBox_" + e)
		...
		position : function() {
			var h, x = 0, k = 0, m = 0, y = 0, s, o, e, E, u, q, f = this.target
					.data("height"), C = this.target.data("width"), r = a(
					window).scrollTop(), B = parseInt(this.s.offsets.x, 10) || 0, A = parseInt(
					this.s.offsets.y, 10) || 0, w = this.cacheData;
			if (!f) {
			...
			var g = [ "4-1", "1-4", "5-7", "2-3", "2-1", "6-8", "3-4", "4-3",
					"8-6", "1-2", "7-5", "3-2" ], v = this.s.position, d = false, j;
			...
			var t = function(H) {
				var I = 0, F = 0, G = (c.s.sharpAngle && c.corner) ? true
						: false;
				if (H === "right") {
					F = s + k + C + B;
					if (G) {
		...
		createSharp : function(g) {
			...
			var h = '<div id="floatCorner_' + g
					+ '" class="float_corner float_corner_' + g
					+ '"><span class="corner corner_1" ' + f
					+ '>◆</span><span class="corner corner_2" ' + d
					+ ">◆</span></div>";
